...
function retrieveRules(query: string) {
  // data/ folder is at the same root as the app - process.cwd() points to repo root on Vercel
  ...
  try {
    ...
    console.error("Failed to load disposal_rules.json at", filePath, e);
    // Built-in fallback rules so the app never crashes
    rules = [
